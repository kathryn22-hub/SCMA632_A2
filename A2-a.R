...
sum(is.na(data$Meals_At_Home))
sum(is.na(data$No_of_Meals_per_day))
...
model <- lm(foodtotal_q~ MPCE_MRP+MPCE_URP+Age+Meals_At_Home+Possess_ration_card+Education+No_of_Meals_per_day, data = subset_data)
...
for (i in 2:length(coefficients)) {
  equation <- paste0(equation, " + ", round(coefficients[i], 8), "*x", i-1)
}
# Print the equation
print(equation)
...
head(subset_data$No_of_Meals_per_day,1)
